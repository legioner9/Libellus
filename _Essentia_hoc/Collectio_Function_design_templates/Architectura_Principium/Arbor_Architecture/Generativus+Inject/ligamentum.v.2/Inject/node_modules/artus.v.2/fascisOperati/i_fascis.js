...
const objProto = {
  clear() {
    this.getCommunis().props.cache.clear();
  },
  add(key, err, data) {
    this.emit('add', err, data);
    this.getCommunis().props.cache.set(key, { err, data });
    return this;
  },
  del(key) {
    this.emit('del', key);
    this.getCommunis().props.cache.delete(key);
    return this;
  },
  get(key, callback) {
    const record = this.getCommunis().props.cache.get(key);
    callback(record.err, record.data);
    return this;
  },
  on(eventName, listener) {

    if (eventName in this.getCommunis().props.events) {
      this.getCommunis().props.events[eventName] = listener;
    }
  },
  emit(eventName, ...args) {
    const event = this.getCommunis().props.events[eventName];
    if (event) event(...args);
  }
};
...
const internus = function(memoized, communis, fn, ...args) {
  ...
  fn(...args, (err, data) => {
    ...
    if (communis.props.cache.size > communis.props.lengthCache) {
...
objProto.setLengthCache = (length) => communis.props.lengthCache = length;
